...
revbox = player.Player(pygame.draw.rect(screen, dark_red, pygame.Rect(x-120,y-120, 60,30)))
all_sprites = pygame.sprite.Group()
all_sprites.add(rev)
all_sprites.add(revbox)
revboxVal = False
revMovement = True
while not done:
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN:#Add something for touching reveille
            if revbox.rect.collidepoint(event.pos):
                revboxVal = True
    ...
    movement.RevUpdate(revbox,revboxVal)
